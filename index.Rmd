...
# .white[Vorlesung Statistik:]

.em14[<i class="fas fa-chart-bar fa-5x"></i>]

### .white[Kreuztabellen und Einführung in die Inferenzstatistik]
...
<br>
<br><br>
$$\frac{P(data | H_0)}{P(data|H_1)} = \frac{Bin(pos, neg, 0.31)}{\int Bin(pos, neg, \theta) \cdot \pi(\theta)\;d\theta} := \text{Bayes Faktor}_{01} = BF_{01}$$

<br><br>

$$\frac{P(data|H_1)}{P(data | H_0)} = \frac{\int Bin(pos, neg, \theta) \cdot \pi(\theta)\;d\theta}{Bin(pos, neg, 0.31)} := \text{Bayes Faktor}_{10} = BF_{10}$$


--
Der $BF_{01}$ quantifiziert relative Evidenz. Er hat einen Wertebereich von 0 bis $+\infty$, wobei 0 die maximale Evidenz für $H_1$, $+\infty$ die maximale Evidenz für $H_0$ und 1 maximale Inkonklusivität zum Ausdruck bringt.

Der $BF_{10}$ quantifiziert relative Evidenz. Er hat einen Wertebereich von 0 bis $+\infty$, wobei 0 die maximale Evidenz für $H_0$, $+\infty$ die maximale Evidenz für $H_1$ und 1 maximale Inkonklusivität zum Ausdruck bringt.
